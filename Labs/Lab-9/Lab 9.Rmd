...
```{r, eval=T}
data_100 <- matrix(rbinom(N * 100, k, p), N)
E100 <- (apply(data_100, 1, est_kp))
data_250 <- matrix(rbinom(N * 250, k, p), N)
E250 <- (apply(data_250, 1, est_kp))
...
Bias lowers with sample size, as does MSE. As expected.
...
```{r, eval=T}
ggplot(
  data = filter(df_k, Estimates >= 0 & Estimates <= 50), 
