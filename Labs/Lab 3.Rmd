...
flight.summary = 
  ...
  mutate(
    ...
    accuracy      = sqrt(abs(dep_delay_min)^2 + abs(arr_delay_min)^2)
  ) %>%
  ...
  summarize(
    ...
    mean.dist = mean(distance), .groups = 'drop'
  ) %>%
...
```{r,eval=T}
ggplot(data = flight.summary, aes(x = mean.dist, y = mean.acc, color = origin, group = origin)) +
  geom_point() +
...
```{r,eval=T}
ggplot(data = flight.summary %>% filter(mean.dist <= 3000), aes(x = mean.dist, y = mean.acc, color = origin, group = origin)) +
  geom_point() +
...
Having JFK as the origin is the worst. As flights from JFK go greater distances, the average accuracy increases (which is bad). A higher accuracy means that flight delays are longer.
...
flight.summary2 %>%
  kbl() %>%
  kable_classic(full_width = F, html_font = "Comic Sans MS")
```
...
1) For the last leg of a trip, I will be in New York for my connection. I can be late to my desitantion, but I dont want to get stuck in any of the airports. What carrier has the most consistent and accurate departure time. 



2) As a theoretical person who has to travel in and out of New York (North East Headquarters) along with Atlanta, Salt Lake City, and Portland (SE, SW, and NW headquarters, respectively), I want to know how accurate flights to and from all four cities are, to make my work flights as painless as possible. I also want to stick to one carrier for all my travel to maximize my airpoints. I could find this data from a whole host of open data flight trackers online.
