...
```{r,eval=F}
f1d<-arrange(f1c, "carrier", "origen", "destination")
head(f1d)
```
...
```{r,eval=F}
f1e<-arrange(f1d, desc("carrier"), desc("origen"), desc("destination"))
head(f1e)
```
...
May we conclude that flight numbers can be mapped to a unique carrier? Yes (*Yes/No*)

Why or why not?

It seems that this number is only for B6, therefore it carries that each number is only for one carrier.

If we know the flight number, carrier, and origin, can we know for sure what the destination will be? Yes (*Yes/No*)

Why or why not?

Assuming we know the destination of a flight with those markers, we can assume that all flights with that information will be going the same place.
...
```{r,eval=F}
# i feel like this is the way I would interpret gain to mean, so this is the order I went with. Especially because it follows the practice of the previous question.
f2c=mutate(f2b,
...
#gain is how much time was lost in the air.
head(f2d)
```
