...
```{r}
spreaded <- spread(top, `lgID`, teamID)
head(spreaded, 22)
```

```{r}
gatherered <- gather(spreaded, lgID, teamID, `AL`:`NL`, na.rm=T)
gatherered
```
...
top2 = Salaries %>%
  ...
  select(`playerID`,`FirstName` = `nameFirst`, `LastName` = `nameLast`, `Team` = `teamID`, `Homeruns` = `HR`, `Salary` = `salary`) %>%
  arrange(desc(Salary))
...
```
Answer: For a flight to be missing a tailnum, it implies that the flight never took of, despite being scheduled. Reason as to why is unclear.
...
Answer: MQ and AA are carriers that dont have tail numbers.