...
```{r, eval = T}
Power <- function(numTrial, n, mu, sd) {
  count <- 0
  for (i in 1:numTrial) {
...
powerVector <- sapply(effect, function(mu) {
  Power(numTrial = 10000, n = 4, mu = mu, sd = 1)
})
...
Its a inverted bell curve basically. As $\mu$ increases, we see logarithmic growth, flattening out at 1. At $\mu=0$, the power is between 0 and .1. Both curves agree, as expected.
...
```{r, eval = T}
powerVector1a <- sapply(effect, function(mu) Power(numTrial = 10000, n = 4, mu = mu, sd = 1))
powerVector1b <- sapply(effect, function(mu) Power(numTrial = 10000, n = 6, mu = mu, sd = 1))
powerVector1c <- sapply(effect, function(mu) Power(numTrial = 10000, n = 8, mu = mu, sd = 1))
...
The test becomes more powerful as the sample size increases
...
```{r, eval = T}
powerVector2a <- sapply(effect, function(mu) { Power(numTrial = 10000, n = 4, mu = mu, sd = 1.)})
powerVector2b <- sapply(effect, function(mu) { Power(numTrial = 10000, n = 4, mu = mu, sd = 1.5)})
powerVector2c <- sapply(effect, function(mu) { Power(numTrial = 10000, n = 4, mu = mu, sd = 2.)})
```
...
This does follow what I expected
