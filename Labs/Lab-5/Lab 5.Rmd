...
The most common value for years of experience is 9 and occurs 654 times.
...
The maximum number of years in school was 16 years which occurred 16 times in our sample
...
People who don't go to school are instead working, getting experience.
...
They are basically the same, with male being slightly offset to the right, showing the wage gap.
...
There is statistical evidence the genders are paid differently because females upper bound is still below males lower bound. Showing that these are statistically different amounts, as their confidence intervals do not overlap.
...
Use `geom_point()` along with the option `color=sex` to overlay scatter plots. Does there seem to be a clear distinction between female and male regarding this relationship? Maybe? pushing no? (yes/no).

```{r}
ggplot(wage, aes(x =experience , y=wage, color = sex)) +
  geom_point()#alpha = .3)
```
...
```{r}
ggplot(wage, aes(x =school , y=wage, color = sex)) +
  geom_point()#alpha = .2)
```
...
Females with low experience and school get far worse pay than anyone else. Men generally get paid more. Men also have a greater spread of school. Femalses dont really seem to get paid more for experience, only school.
...
The female plot is generally darker, showing the standard deviation of wages is lower across different experience and school amounts for females.
...
Females in that situation are pretty locked into a wage they will recieve. Males however have a lot of variety in the wage they may recieve in that same postion. Regardless, the male average for those metrics is higher.