...
for(I in 1:nrow(OUT.MAE)){
  for(J in 1:ncol(OUT.MAE)){
    ...
    DATA3.PRED = DATA3 %>% 
      ...
      select(predict) %>%
      unnest()
    OUT.MAE[I, J] = MAE.func(actual = DATA3.PRED$W, predict = DATA3.PRED$.fitted)
  }
}
...
Code and Output:
```{r,eval=T}
#best_models <- OUT.MAE2.df[order(OUT.MAE.df$MAE), ][1:3, ]

BEST3.MAE <- as_tibble(OUT.MAE2) %>%
  arrange(MAE) %>%
  head(3)
...
BEST3.DATA2 <- BEST3.DATA %>%
  gather(Model, Predict, -D, -L, -W, -A, -T)

BEST3.DATA2$Model <- factor(BEST3.DATA2$Model, levels = c("First", "Second", "Third"), ordered = TRUE)
...
ggplot(BEST3.DATA2[BEST3.DATA2$L == 920, ]) +
  geom_point(aes(x = T, y = W, alpha = 0.05), color = "blue", stroke = 0) +
  geom_line(aes(x = T, y = Predict, color = Model)) +
  labs(x = "Julian Day", y = "Max Water Temperature") +
  scale_alpha_continuous(range = c(0.05, 0.05)) +
  theme_minimal()
```

Answer *(2 Points)*: They barely differ. The first model is slightly lower, and one could argue the first and second are both a little more extreme in some places, but its tiny.
...
Answer: The average marginal change plummets so fast, so we realy dont need 5:9. Nothing is changing. It would be better to jsut run it a couple times, especially because it takes lowkey a while to run. But the first few loops are critical!
...
Code and Output *(3 Points)*:
```{r,warning=F}
OUT.MAEB <- matrix(NA, 9, 9)

train.model.func2 <- function(data, I, J) {
  mod <- lm(W ~ poly(A, I) + poly(D, J) + I(A * D), data = data)
  return(mod) 
}
...
for (I in 1:nrow(OUT.MAE)) {
  for (J in 1:ncol(OUT.MAE)) {
    DATA3B <- DATA2 %>% 
      mutate(tr.model = map(train, train.model.func2, I, J))
    DATA3B.PRE <- DATA3B %>% 
      mutate(predict = map2(test, tr.model, ~augment(.y, newdata = .x))) %>%
      select(predict) %>%
      unnest()
    OUT.MAEB[I, J] <- MAE.func(actual = DATA3B.PRE$W, predict = DATA3B.PRE$.fitted)
  }
}
...
Answer *(2 Points)*: 
yeah, the interaction variable improved the polynomial model. We can compare the model's performance with and without the interaction variable using the mean squared error (MSE). If the model with the interaction term has a lower MSE compared to the model without the interaction term, so this ones better at predicting.
...
OUT.MAEC=matrix(NA,9,9)


Q2.FUNC=function(data,I,J){
  mod=lm(W ~ poly(A,I) + poly(D,J) + L, data=data)
  return(mod) 
}
...
for(I in 1:nrow(OUT.MAE)){
  for(J in 1:ncol(OUT.MAE)){
    ...
    DATA3C = DATA2 %>% 
      mutate(tr.model = map(train, Q2.FUNC, I, J))
    ...
    OUT.MAEC[I, J] = MAE.func(actual = DATA3C.PRED$W, predict = DATA3C.PRED$.fitted)
...
Answer *(2 Points)*: 
Yes, $L$ seems to have improved the polynomial model based on the observed Mean Absolute Error (MAE) values. The MAE values for the model are consistently lower. Which is good. or like at least better at predicting
...
Code and Output:
```{r}
best_poly_model <- lm(W ~ poly(A, 2) + poly(D, 6) + I(A * D), data = DATA)

poly_interact_model <- lm(W ~ poly(A, 2) + poly(D, 6) + I(A * D) + L, data = DATA)

poly_L_model <- lm(W ~ poly(A, 2) + poly(D, 6) + I(A * D) + L, data = DATA)
predictions_best_poly <- predict(best_poly_model)

predictions_poly_interact <- predict(poly_interact_model)

predictions_poly_L <- predict(poly_L_model)

DATA <- DATA %>%
  mutate(
    Best_Poly = predictions_best_poly,
    Best_Poly_Interact = predictions_poly_interact,
    Best_Poly_L = predictions_poly_L
  )

DATA_gathered <- DATA %>%
  gather(Model, Predict, Best_Poly:Best_Poly_L)

ggplot(DATA_gathered) +
  geom_point(aes(x = D, y = W, alpha = 0.005, stroke = 0)) +
  geom_line(aes(x = D, y = Predict, color = Model, alpha = .1)) +
  labs(x = "Julian Day", y = "Max Water Temperature") +
  theme_minimal() +
