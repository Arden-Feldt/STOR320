...
Code and Output:
```{r,eval=T}
clean.func = function(Dataset){                                    

  result <- Dataset %>%                                            #DO NOT CHANGE
  mutate(Date = as_date(date)) %>%                                 #DO NOT CHANGE
  separate(date,into=c("Year","Month","Day"),sep="-") %>%          #DO NOT CHANGE
  mutate_at(1:3,as.numeric) %>%                                    #DO NOT CHANGE
  rename("Temperature"="meantemp","Humidity"="humidity",           #DO NOT CHANGE
...
Code and Output:
```{r,eval=T}
month.func <- function(monthVector) {
  results <- character(length(monthVector))
  for (i in seq_along(monthVector)) {
    if (monthVector[i] == 1) {
      results[i] <- "January"
    } else if (monthVector[i] == 2) {
      results[i] <- "February"
    } else if (monthVector[i] == 3) {
      results[i] <- "March"
    } else if (monthVector[i] == 4) {
      results[i] <- "April"
    } else if (monthVector[i] == 5) {
      results[i] <- "May"
    } else if (monthVector[i] == 6) {
      results[i] <- "June"
    } else if (monthVector[i] == 7) {
      results[i] <- "July"
    } else if (monthVector[i] == 8) {
      results[i] <- "August"
    } else if (monthVector[i] == 9) {
      results[i] <- "September"
    } else if (monthVector[i] == 10) {
      results[i] <- "October"
    } else if (monthVector[i] == 11) {
      results[i] <- "November"
    } else if (monthVector[i] == 12) {
      results[i] <- "December"
    } else {
      results[i] <- "Whoops"
    } 
...
print(unique(TRAIN.CLEAN$Month))
print(unique(TEST.CLEAN.2$Month))
...
```{r,eval=T}
  ...
  TRAIN.CLEAN.3$Day = rep(c("Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday", "Monday"), length.out = nrow(TRAIN.CLEAN.3))
  TEST.CLEAN.3$Day  = rep(c("Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday", "Monday"), length.out = nrow(TEST.CLEAN.3))
...
cels.2.fahr <- function(temp_celsius) {
...
for(j in 1:12) {
  for(k in 1:7) {
    ...
    monthCheck <- TRAIN.FINAL$Month == month[j]
    dayCheck <- TRAIN.FINAL$Day == day[k]

    joint_condition <- monthCheck & dayCheck

    mean_humidity <- mean(TRAIN.FINAL$Humidity[joint_condition], na.rm = TRUE)
    sd_humidity <- sd(TRAIN.FINAL$Humidity[joint_condition], na.rm = TRUE)

    LB.HUMID.TRAIN[j, k] <- mean_humidity - 2 * sd_humidity / sqrt(sum(joint_condition))
    UB.HUMID.TRAIN[j, k] <- mean_humidity + 2 * sd_humidity / sqrt(sum(joint_condition))
  }                 #DO NOT CHANGE
}                   #DO NOT CHANGE
...
for(j in 1:12){         #DO NOT CHANGE
  for(k in 1:7){        #DO NOT CHANGE
    ...
    monthCheck <- TEST.FINAL$Month == month[j]
    dayCheck <- TEST.FINAL$Day == day[k]
    joint_condition <- monthCheck & dayCheck

    n_within_CI <- sum(TEST.FINAL$Humidity[joint_condition] >= LB.HUMID.TRAIN[j, k] &
                       TEST.FINAL$Humidity[joint_condition] <= UB.HUMID.TRAIN[j, k], na.rm = TRUE)
    
    total_obs <- sum(joint_condition, na.rm = TRUE)

    # Calculate proportion
    PROP.HUMID.TEST[j, k] <- n_within_CI / total_obs
...
for(j in 2013:2016){                     #DO NOT CHANGE
  for(k in unique(TRAIN.FINAL$Month)){   #DO NOT CHANGE
    ...
    yearCheck <- TRAIN.FINAL$Year == j
    monthCheck <- TRAIN.FINAL$Month == k
    joint_condition <- yearCheck & monthCheck
    
    skewness <- (mean(TRAIN.FINAL$Temperature[joint_condition], na.rm = TRUE) - 
                 median(TRAIN.FINAL$Temperature[joint_condition], na.rm = TRUE)) /
                sd(TRAIN.FINAL$Temperature[joint_condition], na.rm = TRUE)
    
    row_index <- which(rownames(SKEW.TEMP.TRAIN) == j)
    col_index <- which(colnames(SKEW.TEMP.TRAIN) == k)
    
    SKEW.TEMP.TRAIN[row_index, col_index] <- skewness
...
for (j in 1:1461) {
  ...
  if (month %in% c("June", "July", "August")) {
    SUMMER.WIND <- c(SUMMER.WIND, TRAIN.FINAL$Wind[j])
  } else if (month %in% c("December", "January", "February")) {
    WINTER.WIND <- c(WINTER.WIND, TRAIN.FINAL$Wind[j])
  }
} 
...
The t-test is being used to compare the wind between the summer and winter. We are checking to see if there is a change between these two seasons.

From the t-test, we can show that there is enough evidence to conclude that wind speed during summer months is different from the wind speed during winter months. THe test showed us that wind speeds during summer months are likely to be different from wind speeds during winter months because the P-value was below a threshhold, in this case .05.
...
ggplot(TRAIN.FINAL, aes(x = Date)) +
  geom_line(aes(y = Temperature, color = "Daily Temperature"), alpha = 0.2) +
  geom_line(aes(y = `Temperature 3-Day Average`, color = "3-Day Moving Average"), linetype = "dashed", alpha = 0.6) +
  geom_line(aes(y = `Temperature 5-Day Average`, color = "5-Day Moving Average"), linetype = "dotted") +
