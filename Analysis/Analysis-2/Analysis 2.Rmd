...
```{r, eval=T}
columns_to_clean <- c("GDP", "PerCapita", "Share")

GDP4 <- GDP3 %>%
  mutate_at(vars(all_of(columns_to_clean)), ~ str_replace(., "\\$", "")) %>%
  mutate_at(vars(all_of(columns_to_clean)), ~ str_replace(., "%", ""))
...
```{r, eval=T}
numeric_columns <- c("GDP", "Population", "PerCapita", "Share")

GDP5 <- GDP4 %>%
  mutate_at(vars(all_of(numeric_columns)), ~ as.numeric(str_replace_all(., ",", "")))
...
```{r, eval=T}
numeric_columns <- setdiff(names(GDP5), "Country")


GDP6 <- GDP5 %>%
  mutate(Growth = as.numeric(sub("%","",Growth))/100)
...
GDP7$GDP <- GDP7$GDP / 1e12
GDP7$Population <- GDP7$Population / 1e6
...
EDU <- read_html(URL.EDU) %>%
  ...
  .[[1]] %>%
  html_table(header = TRUE) %>%
  pivot_longer(cols = matches("^\\d{4}$"), names_to = "Year", values_to = "Ed.Index") %>%
...
#  html_table(header = TRUE)# %>%
  #.[[2]] #%>%
  #select(-Rank) %>%
  #pivot_longer(cols = -Country, names_to = "Year", values_to = "Ed.Index") %>%
...
```{r,eval=T}
URL.EDU <- "https://en.wikipedia.org/wiki/Education_Index"

EDU_filtered <- EDU %>%
  filter(!is.na(Ed.Index) & Year >= 2000 & Year <= 2019)

EDU2 <- EDU_filtered %>%
  group_by(Country) %>%
  summarize(AVG.EDU = mean(Ed.Index), SD.EDU = sd(Ed.Index)) %>%
  filter(!any(is.na(AVG.EDU)) & !any(is.na(SD.EDU))) %>%
  arrange(Country)
...
```{r,eval=T}
num_countries_missing <- nrow(EDU %>%
                                filter(is.na(Ed.Index) & Year >= 2000 & Year <= 2019) %>%
                                distinct(Country))
...
```{r,eval=T}
EDU2_not_in_GDP7 <- anti_join(EDU2, GDP7, by = c("Country"))

# Print the resulting data frame
print(EDU2_not_in_GDP7)
```
...
numeric_columns <- c("2018", "2019", "2020", "2021", "2022")

HEALTH <- URL.HEALTH %>%
  read_html() %>%
  html_table(header = TRUE) %>%
  .[[1]]  %>%
  #select(-`World`, -`Year`, -`% of GDP`, -`Healthcare System`)%>%
  mutate_at(vars(all_of(numeric_columns)), ~ as.numeric(str_replace_all(., ",", ""))) %>%
  mutate(`Health.Change` = as.numeric(`2020`) - as.numeric(`2018`)) %>%
  filter(!is.na(`2020`) || !is.na(`2018`)) %>%
  select(Location, `Health.Change`)
...
##CHECK TO SEE IF THIS IS WORKGING PLZPLZPLZ DONT FORGET YOU SILLY BILLY
library(stringr)
...
#printable  <- inner_join(GDP7, HEALTH2, by = "Country")

#printable

arrange(GDP7, Country)
...
There are secret invisible spaces after lots of the countries. So the join sees them as different characcter arrays and doesn't join them properly. This is similar to the exploit Ender uses in Enders Game to create duplicate names of his classmates (if I remember correctly).
