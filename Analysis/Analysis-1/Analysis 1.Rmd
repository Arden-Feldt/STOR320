...
```{r}
n_last <- 4
salary4 = mutate(salary3, HireYear = as.numeric(substr(Hire, nchar(Hire) - n_last + 1, nchar(Hire))))
#head(salary4)
str(salary4, 5)
```
...
```{r}
salary.final = select(salary5, 3:7, 9) %>% arrange(YrsEmployed, Salary)
head(salary.final, 50)
```
...
Code *(2 Points)*:
```{r}
compVEco <- select(salary.final, Department, Salary)
compVEco.summary<- compVEco %>%
                  group_by(Department) %>%
                  summarize(
                    mean.Salary=mean(Salary,na.rm=TRUE))
filteredCompVEco <- filter(compVEco.summary, Department == "Economics" | Department == "Computer Science")
head(filteredCompVEco)
EcoSal <- subset(filteredCompVEco, Department == "Economics", select = 'mean.Salary')
CompSal <- subset(filteredCompVEco, Department == "Computer Science", select = 'mean.Salary')
difference <- EcoSal-CompSal
print(EcoSal-CompSal)
```

Answer *(1 Point)*: The difference between the average Economics Department Salary and Computer Science Department Salary is 45537.79 dollars USD. This is assuming the question of my department is the one I am in, and not the one proffessor Mario is in.
...
Code *(2 Points)*:
```{r}
#findThisDumbVar <- arrange(salary.final, desc(Salary))
#head(findThisDumbVar)
totalAssistProfs <- filter(salary.final, Category == "Assistant Professor")
ballerAssistProfs <- filter(totalAssistProfs, Salary > 90000)
prroportion <-  nrow(ballerAssistProfs) / nrow(totalAssistProfs)
print(prroportion)
```

Answer *(1 Points)*: Of all assistant professors, 74% (0.7492188) make more than $90k as their annual salary.
...
```{r}
head(salary.final)
#pairedDown <- select(salary.final, Department, Salary, Category)
#pairedDown <- filter(pairedDown, Category == "Instructor" | Category == "Lecturer")
#head(pairedDown)
#trying <- pairedDown %>%
#          group_by(Department)
#head(trying)
#trying <- filter(trying, n_distinct(unique(Department)) >= 3) 
#head(trying)

#deptsOverThree <- filter(trying, n >= 3)
#deptsOverThreeverThree(again, 30)
#salaryMath <- pairedDown %>%
#                  group_by(Department) %>%
#                  summarise(
#                    mean.Salary=mean(Salary,na.rm=TRUE)
#                  )
...
result <- clone %>%
  ...
  group_by(Department) %>%
  summarise(num_teaching_employees = n(),
            total_spending = sum(Salary)) %>%
  filter(num_teaching_employees >= 3) %>%
...
Answer *(1 Points)*: "Psychiatry - General" spends the most money on educators. 
...
dept_cat_summary <- salary.final %>%
  ...
  summarise(n = n(),
            ...
            age_vs_salary = as.integer(cor(Age, Salary)), .groups = 'drop')

print(dept_cat_summary)
```
...
Code:
```{r}
weakest_correlations <- dept_cat_summary %>%
  filter(n >= 10) %>%
  arrange(age_vs_salary) %>%
  head(5) %>%
  select(Department, Category, age_vs_salary)

#head(weakest_correlations, 12) There are only 5
...
```{r}
professor_plot_data <- dept_cat_summary %>%
  filter(Category == "Professor", n >= 5)

ggplot(professor_plot_data, aes(x = yrs, y = salary)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, col = "blue", fill = "lightblue") +
  labs(title = "Relationship between Average Years Employed and Average Salary of Professors",
       x = "Average Years Employed",
       y = "Average Salary")
```

Answer *(2 Points)*: There is a weak negative correlation. Which is really weird and confusing, I would assume salary would increase with time, but maybe they get fired. 
...
Code and Figure: 
```{r}
selected_departments <- dept_cat_summary %>%
  filter(Department %in% c("Dermatology - Adult", "Comparative Medicine", "Classics")) %>%
  slice_sample(n = 3)

ggplot(dept_cat_summary %>% filter(Department %in% selected_departments$Department), aes(x = yrs, y = salary)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, col = "blue") +
  labs(title = "Relationship between Average Years Employed and Average Salary",
       x = "Average Years Employed",
       y = "Average Salary") +
...
```{r}
filtered_departments <- dept_cat_summary %>%
  filter(Category %in% c("Professor", "Associate Professor", "Assistant Professor")) %>%
  group_by(Department) %>%
  filter(n_distinct(Category) == 3) %>%
  ungroup() %>%
  slice_sample(n = 3)

# Create a tile plot for the selected departments and specified categories
ggplot(dept_cat_summary %>% filter(Department %in% filtered_departments$Department & Category %in% c("Professor", "Associate Professor", "Assistant Professor")),
       aes(x = Department, y = Category, fill = salary)) +
  geom_tile() +
  labs(title = "Average Salary by Department and Category",
       x = "Department",
       y = "Category",
       fill = "Average Salary") +
